...
ContextMenu_Edit(input)
{
  ...
  if urls.Length > 0
  {
    ...
    ctxUrl.Add("Copy as Markdown", () => View_Text(Stream(urls).ToArray(url => ConvertTo_MarkdownLink(Url_Decode(url)))))
    ctxUrl.Add("Copy as Excel", () => View_Text(Stream(urls).ToArray(url => ConvertTo_ExcelHyperlink(Url_Decode(url)))))
  ...
  if links.Length > 0
  {
    ...
    if links.Length == 1
    {
      ...
      ctxLink.Add("Copy Link as Markdown", () => View_Text(ConvertTo_MarkdownLink(link.href, link.text)), "md")
      ctxLink.Add("Copy Link as Excel", () => View_Text(ConvertTo_ExcelHyperlink(link.href, link.text)))
    }
    else
    {
      ctxEdit := ContextMenu()
      ...
      ctxLink.Add("Copy Link as Markdown List", () => View_Text(Stream(links).ToArray(link => "- " ConvertTo_MarkdownLink(link.href, link.text)), "md"))
      ctxLink.Add("Copy Link as Markdown List (Ordered)", () => View_Text(Stream(links).ToArray(link => "1. " ConvertTo_MarkdownLink(link.href, link.text)), "md"))
      ctxLink.Add("Copy Link as Excel", () => View_Text(Stream(links).ToArray(link => ConvertTo_ExcelHyperlink(link.href, link.text))))
    }
    ctxLink.AddSeparator()
...
ContextMenu_ConvertTo(input)
{
  ...
  ctxConvertTo.Add("AutoHotkey", () => View_Text(ConvertTo_AutoHotkey(input)))
